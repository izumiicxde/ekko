...
public class DocumentAdapter extends ListAdapter<DocumentEntity, DocumentAdapter.DocumentViewHolder> {
    ...
    private static final DiffUtil.ItemCallback<DocumentEntity> DIFF_CALLBACK =
            new DiffUtil.ItemCallback<DocumentEntity>() {
                @Override
                public boolean areItemsTheSame(@NonNull DocumentEntity a, @NonNull DocumentEntity b) {
                    return a.id == b.id;
                }

                @Override
                public boolean areContentsTheSame(@NonNull DocumentEntity a, @NonNull DocumentEntity b) {
    ...
    static class DocumentViewHolder extends RecyclerView.ViewHolder {
        ...
        void bind(DocumentEntity doc, OnDocumentClickListener listener) {
            ...
            if (doc.keywords != null && !doc.keywords.isEmpty()) {
                ...
                int max = Math.min(keywords.length, 4);
                for (int i = 0; i < max; i++) {
                    ...
                    chip.setTypeface(ResourcesCompat.getFont(ctx, R.font.bricolage_grotesque));
        ...
        private String formatWordCount(int count) {
            if (count >= 1000) {
                return String.format("%.1fk words", count / 1000f);
            }
            return count + " words";
        }
    }
